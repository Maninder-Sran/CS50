...
char* indexes(const char* path)
{
   char* html_index = (path[strlen(path) - 1] == '/') ? "index.html" : "/index.html";
   char* php_index = (path[strlen(path) - 1] == '/') ? "index.php" : "/index.php";
   
   char* path_html = malloc(strlen(path) + strlen(html_index));
   if (path_html == NULL)
        return NULL;
   strcpy(path_html, path);
   strcat(path_html, html_index);
   
   char* path_php  = malloc(strlen(path) + strlen(php_index));
   if (path_php == NULL)
        return NULL;
   strcpy(path_php, path);
   strcat(path_php, php_index);
   
   if (access(path_php, F_OK) == 0)
        return path_php;
    else if (access(path_html, F_OK) == 0)
        return path_html;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    for(int i = 0; i < HTTP_length - 1; i++)
    {
        HTTP_version[i] = line[index + size + 2 + i];
    } 
    HTTP_version[HTTP_length - 1] = '\0';
    printf("%s<-\n",HTTP_version);
